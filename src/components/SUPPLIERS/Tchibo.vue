<template>
  ...
  <v-app id="inspire">
    <h1  align="center" color="grey">Tchibo</h1>
    <v-card>
        <v-card-title>
            <v-text-field
            v-model="search"
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Purchased Oty", value: "received_qty_ea" }
      ]
    };
  },
  notifications: {
  ...
  methods: {
    fetchData() {
      this.loading = true
       ...
       let query = dbRef.orderByChild("supplier").equalTo("Tchibo")
      query.on(
        "value",
        response =>{
      ...
      // axios
      //   .get("http://localhost:3000/tchibo")
